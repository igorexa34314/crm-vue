<template>
	<div>
					...
					<v-card
						...
						<v-card-text class="text-subtitle-1 text-primary">
							...
							<div v-if="recordWithCategory.details?.length" class="record__details mt-4">
								...
								<p
									v-for="detail in recordWithCategory.details"
									:key="detail.fullname.split('/').at(-1)?.split('.')[0]"
									class="record__detail mb-2 text-fixed">
							...
							<small class="text-right d-block mr-1">{{ d(recordWithCategory.date, 'short') }}</small>
						</v-card-text>
					</v-card>
				</v-col>
			</v-row>
		...
		<div v-else class="mt-7 text-center text-primary text-h6">
			<strong>
				Записи с id: <span class="text-decoration-underline font-italic"> {{ route.params.id }}</span>
				не найдено
			</strong>
		</div>
	</div>
...
const recordWithCategory = computedAsync(
	async () => {
		if (route.params.id) {
